...
def test_pattern_outside_chokepoint_is_flagged(tmp_path: Path) -> None:
    """The pattern is forbidden outside the configured chokepoint."""
    _seed(tmp_path, "src/drain.py", _BAD)
    _seed(tmp_path, "src/client.py", _BAD)  # the chokepoint — legitimate here
    rule = build(
        {"roots": ["src"], "patterns": [_PATTERN], "chokepoint_files": ["src/client.py"]},
        repo_root=tmp_path,
    )
...
def test_multiple_patterns_any_match_flags(tmp_path: Path) -> None:
    ...
    rule = build(
        {"roots": ["src"], "patterns": [_PATTERN, r"_is_write_query"], "chokepoint_files": ["src/client.py"]},
        repo_root=tmp_path,
    )
